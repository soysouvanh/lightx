//! # MySQL End-to-End API Demonstration
//!
...
impl DbDemoBo {
    ...
    /// - `POST` requests extract JSON from `ctx.raw_body` to natively insert `MysqlUsers`.
    pub async fn execute(
        ctx: &mut crate::RequestContext,
    ) -> Result<Response<Full<Bytes>>, AppError> {
        // Extract the highly optimized MySQL connection pool from the Request Context.
        ...
        match method {
            Method::GET => {
                ...
                let users = crate::MysqlUsers::list_by_cursor(pool, -1, 100)
                    .await
                    .map_err(map_err)?;
