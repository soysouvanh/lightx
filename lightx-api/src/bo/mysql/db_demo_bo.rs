...
impl DbDemoBo {
    pub async fn execute(
        ...
        crate::MysqlUsers::insert_batch(&mut tx, &batch_users)
            .await
            .map_err(map_err)?;
        tx.commit().await.map_err(map_err)?;

        // --- COUNT ---
        let count = crate::MysqlUsers::approximate_count(pool)
            .await
            .map_err(map_err)?;
        ...
        loop {
            let page = crate::MysqlUsers::list_by_cursor(pool, loop_last_id, 100)
                .await
                .map_err(map_err)?;
            if page.is_empty() {
                break;
        ...
        let mut tx = pool.begin().await.map_err(map_err)?;
        crate::MysqlUsers::delete_many_by_id(&mut tx, &ids)
            .await
            .map_err(map_err)?;
        tx.commit().await.map_err(map_err)?;
