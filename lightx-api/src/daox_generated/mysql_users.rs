...
impl MysqlUsersOrderBy {
    pub fn as_str(&self) -> &'static str {
        match self {
            MysqlUsersOrderBy::EmailAsc => r#"`email` ASC"#,
            MysqlUsersOrderBy::EmailDesc => r#"`email` DESC"#,
            MysqlUsersOrderBy::IdAsc => r#"`id` ASC"#,
            MysqlUsersOrderBy::IdDesc => r#"`id` DESC"#,
            MysqlUsersOrderBy::IsActiveAsc => r#"`is_active` ASC"#,
            MysqlUsersOrderBy::IsActiveDesc => r#"`is_active` DESC"#,
            MysqlUsersOrderBy::RoleIdAsc => r#"`role_id` ASC"#,
            MysqlUsersOrderBy::RoleIdDesc => r#"`role_id` DESC"#,
        }
    }
}
...
impl MysqlUsers {
    ...
    pub async fn upsert_unchecked<'e, E: sqlx::Executor<'e, Database = sqlx::MySql>>(
        ...
        let query = r#"INSERT INTO `users` (`email`, `is_active`, `role_id`) VALUES (?, ?, ?) ON DUPLICATE KEY UPDATE `email` = VALUES(`email`), `is_active` = VALUES(`is_active`), `role_id` = VALUES(`role_id`)"#;
        let result = sqlx::query::<sqlx::MySql>(query)
            .bind(&self.email)
            .bind(&self.is_active)
            .bind(&self.role_id)
    ...
    pub async fn upsert_batch<'e>(
        ...
        for chunk in items.chunks(chunk_size.max(1)) {
            ...
            qb.push(r#" ON DUPLICATE KEY UPDATE `email` = VALUES(`email`), `is_active` = VALUES(`is_active`), `role_id` = VALUES(`role_id`)"#);
            let result = qb.build().execute(&mut **executor).await?;
            total_affected += result.rows_affected();
        }
        Ok(total_affected)
