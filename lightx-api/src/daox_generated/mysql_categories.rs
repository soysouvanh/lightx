...
pub enum MysqlCategoriesOrderBy {
    IdAsc,
    IdDesc,
    NameAsc,
    NameDesc,
...
impl MysqlCategoriesOrderBy {
    pub fn as_str(&self) -> &'static str {
        match self {
            MysqlCategoriesOrderBy::IdAsc => r#"`id` ASC"#,
            MysqlCategoriesOrderBy::IdDesc => r#"`id` DESC"#,
            MysqlCategoriesOrderBy::NameAsc => r#"`name` ASC"#,
            MysqlCategoriesOrderBy::NameDesc => r#"`name` DESC"#,
            MysqlCategoriesOrderBy::ParentIdAsc => r#"`parent_id` ASC"#,
            MysqlCategoriesOrderBy::ParentIdDesc => r#"`parent_id` DESC"#,
        }
    }
}

#[allow(clippy::all)]
impl MysqlCategories {
    ...
    pub async fn upsert_unchecked<'e, E: sqlx::Executor<'e, Database = sqlx::MySql>>(
        ...
        let query = r#"INSERT INTO `categories` (`name`, `parent_id`) VALUES (?, ?) ON DUPLICATE KEY UPDATE `name` = VALUES(`name`), `parent_id` = VALUES(`parent_id`)"#;
        let result = sqlx::query::<sqlx::MySql>(query)
            .bind(&self.name)
            .bind(&self.parent_id)
            .execute(executor)
    ...
    pub async fn upsert_batch<'e>(
        ...
        for chunk in items.chunks(chunk_size.max(1)) {
            ...
            qb.push(r#" ON DUPLICATE KEY UPDATE `name` = VALUES(`name`), `parent_id` = VALUES(`parent_id`)"#);
            let result = qb.build().execute(&mut **executor).await?;
            total_affected += result.rows_affected();
        }
        Ok(total_affected)
...
pub struct MysqlCategoriesPatch {
