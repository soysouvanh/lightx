...
impl SqliteUsersOrderBy {
    pub fn as_str(&self) -> &'static str {
        match self {
            SqliteUsersOrderBy::EmailAsc => r#"`email` ASC"#,
            SqliteUsersOrderBy::EmailDesc => r#"`email` DESC"#,
            SqliteUsersOrderBy::IdAsc => r#"`id` ASC"#,
            SqliteUsersOrderBy::IdDesc => r#"`id` DESC"#,
            SqliteUsersOrderBy::IsActiveAsc => r#"`is_active` ASC"#,
            SqliteUsersOrderBy::IsActiveDesc => r#"`is_active` DESC"#,
            SqliteUsersOrderBy::RoleIdAsc => r#"`role_id` ASC"#,
            SqliteUsersOrderBy::RoleIdDesc => r#"`role_id` DESC"#,
        }
    }
}
...
impl SqliteUsers {
    ...
    pub fn stream_all<'e, E: sqlx::Executor<'e, Database = sqlx::Sqlite> + 'e>(
        ...
        let query = r#"SELECT `email`, `id`, `is_active`, `role_id` FROM `users` ORDER BY `id` ASC LIMIT ?"#;
    ...
    pub async fn get_by_id<'e, E: sqlx::Executor<'e, Database = sqlx::Sqlite>>(
        ...
        let query = r#"SELECT `email`, `id`, `is_active`, `role_id` FROM `users` WHERE `id` = ?"#;
    ...
    pub async fn list_by_cursor<'e, E: sqlx::Executor<'e, Database = sqlx::Sqlite>>(
        ...
        let limit = limit.clamp(1, 10000);
        let query = r#"SELECT `email`, `id`, `is_active`, `role_id` FROM `users` WHERE `id` > ? ORDER BY `id` ASC LIMIT ?"#;
        sqlx::query_as::<_, Self>(query)
            .bind(last_id)
            .bind(limit as i64)
            .fetch_all(executor)
    ...
    pub async fn insert_unchecked<'e, E: sqlx::Executor<'e, Database = sqlx::Sqlite>>(
        ...
        let query = r#"INSERT INTO `users` (`email`, `is_active`, `role_id`) VALUES (?, ?, ?)"#;
    ...
    pub async fn upsert_unchecked<'e, E: sqlx::Executor<'e, Database = sqlx::Sqlite>>(
        ...
        let query = r#"INSERT INTO `users` (`email`, `is_active`, `role_id`) VALUES (?, ?, ?) ON CONFLICT (`id`) DO UPDATE SET `email` = EXCLUDED.`email`, `is_active` = EXCLUDED.`is_active`, `role_id` = EXCLUDED.`role_id`"#;
        let result = sqlx::query::<sqlx::Sqlite>(query)
            .bind(&self.email)
            .bind(&self.is_active)
            .bind(&self.role_id)
    ...
    pub async fn update_unchecked_by_id<'e, E: sqlx::Executor<'e, Database = sqlx::Sqlite>>(
        ...
        let query_str =
            r#"UPDATE `users` SET `email` = ?, `is_active` = ?, `role_id` = ? WHERE `id` = ?"#;
