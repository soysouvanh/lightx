...
pub struct MysqlRoles {
    pub id: i32,
    pub name: String,
}

#[derive(Debug, Clone, Copy, PartialEq, Eq)]
pub enum MysqlRolesOrderBy {
    IdAsc,
    IdDesc,
    NameAsc,
    NameDesc,
...
impl MysqlRolesOrderBy {
    pub fn as_str(&self) -> &'static str {
        match self {
            MysqlRolesOrderBy::IdAsc => r#"`id` ASC"#,
            MysqlRolesOrderBy::IdDesc => r#"`id` DESC"#,
            MysqlRolesOrderBy::NameAsc => r#"`name` ASC"#,
            MysqlRolesOrderBy::NameDesc => r#"`name` DESC"#,
        }
    }
}
...
impl MysqlRoles {
    ...
    pub async fn upsert_unchecked<'e, E: sqlx::Executor<'e, Database = sqlx::MySql>>(
        ...
        let query = r#"INSERT INTO `roles` (`name`) VALUES (?) ON DUPLICATE KEY UPDATE `name` = VALUES(`name`)"#;
        let result = sqlx::query::<sqlx::MySql>(query)
            .bind(&self.name)
            .execute(executor)
            .await?;
