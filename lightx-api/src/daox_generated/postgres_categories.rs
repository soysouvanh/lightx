...
pub enum PostgresCategoriesOrderBy {
    IdAsc,
    IdDesc,
    NameAsc,
    NameDesc,
...
impl PostgresCategoriesOrderBy {
    pub fn as_str(&self) -> &'static str {
        match self {
            PostgresCategoriesOrderBy::IdAsc => r#""id" ASC"#,
            PostgresCategoriesOrderBy::IdDesc => r#""id" DESC"#,
            PostgresCategoriesOrderBy::NameAsc => r#""name" ASC"#,
            PostgresCategoriesOrderBy::NameDesc => r#""name" DESC"#,
            PostgresCategoriesOrderBy::ParentIdAsc => r#""parent_id" ASC"#,
            PostgresCategoriesOrderBy::ParentIdDesc => r#""parent_id" DESC"#,
        }
    }
}

#[allow(clippy::all)]
impl PostgresCategories {
    ...
    pub fn validate(&self) -> Result<(), Vec<String>> {
        ...
        if let Some(v) = Some(&self.name) {
            if v.len() > 100 {
                errors.push("name: exceeds max_length 100".into());
            }
        }
        #[cfg(feature = "validation")]
        if let Some(v) = Some(&self.name) {
            static RE: std::sync::OnceLock<Option<regex::Regex>> = std::sync::OnceLock::new();
            let re = RE.get_or_init(|| regex::Regex::new("^.*$").ok());
            match re {
                Some(re) => {
                    if !re.is_match(v) {
        ...
        if let Some(v) = self.parent_id.as_ref() {
            if (*v as i128) < (0 as i128) {
                errors.push("parent_id: minimum value '0' not met".into());
            }
        }
        if let Some(v) = self.parent_id.as_ref() {
            if (*v as i128) > (2147483647 as i128) {
                errors.push("parent_id: maximum value '2147483647' exceeded".into());
            }
        }
        if errors.is_empty() {
            Ok(())
    ...
    pub async fn count<'e, E: sqlx::Executor<'e, Database = sqlx::Postgres>>(
        ...
        let query = r#"SELECT COUNT(*) FROM "categories""#;
    ...
    pub fn stream_all<'e, E: sqlx::Executor<'e, Database = sqlx::Postgres> + 'e>(
        ...
        let query =
            r#"SELECT "id", "name", "parent_id" FROM "categories" ORDER BY "id" ASC LIMIT $1"#;
    ...
    pub async fn get_by_id<'e, E: sqlx::Executor<'e, Database = sqlx::Postgres>>(
        ...
        let query = r#"SELECT "id", "name", "parent_id" FROM "categories" WHERE "id" = $1"#;
    ...
    pub async fn exists_by_id<'e, E: sqlx::Executor<'e, Database = sqlx::Postgres>>(
        ...
        let query = r#"SELECT 1 FROM "categories" WHERE "id" = $1 LIMIT 1"#;
    ...
    pub async fn list_by_cursor<'e, E: sqlx::Executor<'e, Database = sqlx::Postgres>>(
        ...
        let limit = limit.clamp(1, 10000);
        let query = r#"SELECT "id", "name", "parent_id" FROM "categories" WHERE "id" > $1 ORDER BY "id" ASC LIMIT $2"#;
        sqlx::query_as::<_, Self>(query)
            .bind(last_id)
            .bind(limit as i64)
            .fetch_all(executor)
    ...
    pub async fn insert_unchecked<'e, E: sqlx::Executor<'e, Database = sqlx::Postgres>>(
        ...
        let query = r#"INSERT INTO "categories" ("name", "parent_id") VALUES ($1, $2) RETURNING "id"::bigint"#;
    ...
    pub async fn insert_batch<'e>(
        ...
        let mut copy_in = executor
            .copy_in_raw(r#"COPY "categories" ("name", "parent_id") FROM STDIN WITH (FORMAT csv)"#)
            .await?;
    ...
    pub async fn upsert_unchecked<'e, E: sqlx::Executor<'e, Database = sqlx::Postgres>>(
        ...
        let query = r#"INSERT INTO "categories" ("name", "parent_id") VALUES ($1, $2) ON CONFLICT ("id") DO UPDATE SET "name" = EXCLUDED."name", "parent_id" = EXCLUDED."parent_id""#;
    ...
    pub async fn upsert_batch<'e>(
        ...
        for chunk in items.chunks(chunk_size.max(1)) {
            let mut qb: sqlx::QueryBuilder<sqlx::Postgres> =
                sqlx::QueryBuilder::new(r#"INSERT INTO "categories" ("name", "parent_id") "#);
            qb.push_values(chunk, |mut b, item| {
                b.push_bind(&item.name);
                b.push_bind(&item.parent_id);
            });
            qb.push(r#" ON CONFLICT ("id") DO UPDATE SET "name" = EXCLUDED."name", "parent_id" = EXCLUDED."parent_id""#);
    ...
    pub async fn update_unchecked_by_id<'e, E: sqlx::Executor<'e, Database = sqlx::Postgres>>(
        ...
        let query_str = r#"UPDATE "categories" SET "name" = $1, "parent_id" = $2 WHERE "id" = $3"#;
        let mut query = sqlx::query::<sqlx::Postgres>(query_str);
        query = query.bind(&self.name);
        query = query.bind(&self.parent_id);
    ...
    pub async fn delete_by_id<'e, E: sqlx::Executor<'e, Database = sqlx::Postgres>>(
        ...
        let query = r#"DELETE FROM "categories" WHERE "id" = $1"#;
        let result = sqlx::query::<sqlx::Postgres>(query)
            .bind(id)
            .execute(executor)
            .await?;
    ...
    pub async fn delete_many_by_id<'e>(
        ...
        for chunk in ids.chunks(chunk_size) {
            let mut qb: sqlx::QueryBuilder<sqlx::Postgres> =
                sqlx::QueryBuilder::new(r#"DELETE FROM "categories" WHERE "id" IN "#);
            qb.push("(");
    ...
    pub async fn update_partial_by_id<'e, E: sqlx::Executor<'e, Database = sqlx::Postgres>>(
        ...
        if let Some(val) = &patch.parent_id {
            if let Some(v) = val.as_ref() {
                if (*v as i128) < (0 as i128) {
                    errors.push("parent_id: minimum value '0' not met".into());
                }
                if (*v as i128) > (2147483647 as i128) {
                    errors.push("parent_id: exceeds max_value '2147483647'".into());
                }
            }
        }
        if !errors.is_empty() {
...
pub struct PostgresCategoriesPatch {
