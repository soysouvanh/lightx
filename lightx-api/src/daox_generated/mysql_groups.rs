...
impl MysqlGroupsOrderBy {
    pub fn as_str(&self) -> &'static str {
        match self {
            MysqlGroupsOrderBy::IdAsc => r#"`id` ASC"#,
            MysqlGroupsOrderBy::IdDesc => r#"`id` DESC"#,
            MysqlGroupsOrderBy::NameAsc => r#"`name` ASC"#,
            MysqlGroupsOrderBy::NameDesc => r#"`name` DESC"#,
        }
    }
}
...
impl MysqlGroups {
    ...
    pub async fn upsert_unchecked<'e, E: sqlx::Executor<'e, Database = sqlx::MySql>>(
        ...
        let query = r#"INSERT INTO `groups` (`name`) VALUES (?) ON DUPLICATE KEY UPDATE `name` = VALUES(`name`)"#;
        let result = sqlx::query::<sqlx::MySql>(query)
            .bind(&self.name)
            .execute(executor)
            .await?;
